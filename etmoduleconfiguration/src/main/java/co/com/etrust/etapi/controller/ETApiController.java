...
public class ETApiController {
	...
	@RequestMapping(value = "/etapi/presale/productreview/ratecomment", method = RequestMethod.POST,
            headers = {"Content-type=application/json"})
	public @ResponseBody Object rateReview(@RequestBody final ETReviewRateDTO review) {

		try{
		return new ETApiResponseDTO(presaleService.rateReview(review), "Ok",
				ETApiResponseStatus.SUCCESS);
		}catch(Exception e){
			e.printStackTrace();
			return new ETApiResponseDTO(null, "FAIL :: "+e.getMessage(),
